  ...
  return (
    <DefaultLayout title="Portfolio" description={data.hero.subtext}>
      <section className="relative h-[80vh] flex flex-col justify-center items-center text-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,rgba(167,139,250,0.25),transparent_70%)]" />
        ...
        >
          <h1 className="text-5xl font-display font-semibold text-white drop-shadow-[0_2px_20px_rgba(61,90,254,0.3)] mb-4">
            {data.hero.headline}
          </h1>
          ...
          <Link
            ...
            className="px-8 py-3 rounded-full bg-white text-primary font-semibold shadow-md hover:shadow-glow hover:scale-105 transition-all inline-block"
          >
            {data.hero.cta.label}
          </Link>
        </motion.div>
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm transition-all border ${
                  f === filter
                    ...
                    : "border-border text-secondary hover:bg-hover-bg"
                }`}
              >
                {f}
              </button>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8">
            {filtered.map((p: any, i: number) => (
              <motion.div
                ...
                className="relative group rounded-2xl overflow-hidden shadow-soft hover:shadow-glow border border-border transition-all"
              >
                <Link to={`/portfolio/${p.slug}`}>
                  ...
                  />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex flex-col justify-end p-6 transition-opacity duration-500">
          ...
          {data.caseStudies.map((c: any, i: number) => (
            ...
            >
              <img
                src={c.image}
                alt={c.title}
                className="w-full md:w-1/2 rounded-2xl shadow-soft border border-border"
              />
      ...
      <section className="py-24 text-center text-white relative overflow-hidden">
        ...
        >
          ...
          <Link
            ...
            className="px-8 py-3 rounded-full bg-white text-primary font-semibold shadow-md hover:shadow-glow transition-all inline-block"
          >
